...
Use this checklist when you **add or change** platform behavior so CLI, IDE, NL, and CI stay aligned. Canonical repo layout: [`CANONICAL_REPO_AND_INTERFACES`](CANONICAL_REPO_AND_INTERFACES.md).
...
After edits, run from `ai-sdlc-platform/` root:
...
## 8. Public GitHub mirror (same code, neutral docs)

Engineering **canonical** remote: Azure DevOps — see [`GitHub_Metadata`](../GitHub_Metadata.md). The **GitHub** repo is a public mirror (no internal org/project in clone URLs; curated overlays for a few hub pages).

After you commit and push on Azure:

1. Clone or pull the GitHub repo locally (e.g. sibling folder `AI_SDLC_Platform`).
2. From the **Azure** repo root run:  
   `bash scripts/mirror-public/sync-to-public-mirror.sh /path/to/AI_SDLC_Platform`  
   (Omit the path to use `../AI_SDLC_Platform` next to the Azure clone.)
3. In the mirror: `node User_Manual/build-manual-html.mjs`, then `git add -A`, commit, `git push origin main`.

Details, dry-run, and env vars: [`scripts/mirror-public/README.md`](../scripts/mirror-public/README.md).  
**Overlay** files (public-only wording) live under **`scripts/mirror-public/overlays/`** in the Azure repo — edit those when GitHub-facing text must differ from Azure.
