...
```markdown
# {STACK} Variant: Stage {STAGE_NUMBER}

**Stage**: {STAGE_NAME}  
**Stack**: {STACK}  
**Variant Owner**: {PRIMARY_ROLE} (e.g., Backend, Frontend, QA)  
**Token Budget**: {EXPECTED_TOKENS}  
**Updated**: 2026-04-11  
...
Contact: [{VARIANT_OWNER}](mailto:owner@example.com)
```
...
for stage_dir in stages/*/; do
  ...
  for stack in java-backend kotlin-android swift-ios react-native jmeter-perf figma-design; do
    ...
    cat VARIANT_TEMPLATE.md | \
      sed "s/{STAGE_NUMBER}/$stage_num/g" | \
      sed "s/{STAGE_NAME}/$stage_full/g" | \
      sed "s/{STACK}/$stack/g" | \
      sed "s/{VARIANT_OWNER}/[TBD]/g" | \
      sed "s/{EXPECTED_TOKENS}/5000/g" \
      > "$variant_file"
